 ...
 * ../config/arcgis-enterprise-s3files-${var.arcgis_version}.json index file.
 * 
...
locals {
  s3_files_json_path =  "${path.root}/../config/arcgis-enterprise-s3files-${var.arcgis_version}.json"
...
build {
  ...
  provisioner "shell-local" {
    ...
    command = "python -m ssm_run_chef -s ${var.site_id} -d ${var.deployment_id} -m ${local.machine_role} -j /arcgis/${var.site_id}/attributes/arcgis-enterprise-base/image/${var.arcgis_version}/${var.os}/patches -b ${data.amazon-parameterstore.s3_logs.value} -e 1200"
  }
