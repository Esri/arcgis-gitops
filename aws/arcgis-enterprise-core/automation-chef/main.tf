...
resource "aws_ssm_parameter" "chef_client_urls" {
  ...
  name  = "/arcgis/${var.enterprise_id}/chef-client-url/${each.key}"
  type  = "String"
...
resource "aws_ssm_parameter" "chef_client_log_level" {
  name  = "/chef/${var.enterprise_id}/log_level"
  type  = "String"
  value = "info"
