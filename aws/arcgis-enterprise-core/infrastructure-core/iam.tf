...
# IAM role of the enterprise's SSM managed EC2 instances
...
# IAM instance profile of the enterprise's SSM managed EC2 instances
resource "aws_iam_instance_profile" "arcgis_enterprise_profile" {
  name_prefix = var.enterprise_id
  role        = aws_iam_role.arcgis_enterprise_role.name
}
