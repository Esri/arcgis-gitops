...
resource "aws_ssm_parameter" "vpc_id" {
  ...
  description = "VPC ID of ArcGIS Enterprise '${var.enterprise_id}'"
}

resource "aws_ssm_parameter" "hosted_zone_id" {
  name        = "/arcgis/${var.enterprise_id}/vpc/hosted-zone-id"
  type        = "String"
  value       = aws_route53_zone.private.id
  description = "Private hosted zone ID of ArcGIS Enterprise '${var.enterprise_id}'"
}
...
resource "aws_ssm_parameter" "backup_vault_name" {
  ...
  type        = "String"
  value       = aws_backup_vault.enterprise.name
...
resource "aws_ssm_parameter" "images_parameters" {
  ...
  name  = "/arcgis/${var.enterprise_id}/images/${each.key}"
  type  = "String"
  value = each.value.id
...
resource "aws_ssm_parameter" "sns_topic" {
  ...
  type        = "String"
  value       = aws_sns_topic.enterprise_alarms.arn
  description = "Enterprise alarms SNS topic ARN"
}
