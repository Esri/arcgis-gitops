...
do
    ...
    if [ $? -ne 0 ]
    then
        aws ecr create-repository --repository-name "${REPOSITORY_NAME}" --image-scanning-configuration scanOnPush=true --image-tag-mutability IMMUTABLE
        echo "ECR repository '${REPOSITORY_NAME}' created."
    else
        echo "ECR repository '${REPOSITORY_NAME}' already exists."
    fi    

    aws ecr describe-images --repository-name $REPOSITORY_NAME --image-ids imageTag=$TAG > /dev/null 2>&1

    # Copy image to the ECR repository if it does not exist
    if [[ $? == 0 ]]; then
        echo "Image $REPOSITORY_NAME:$TAG is already in the ECR repository"
    else
        set -e

        docker pull -q $REPOSITORY_NAME:$TAG

        IMAGE=$ECR_REGISTRY_URL/$REPOSITORY_NAME:$TAG
        ...
        echo "Image $REPOSITORY_NAME:$TAG copied."
    fi
done
