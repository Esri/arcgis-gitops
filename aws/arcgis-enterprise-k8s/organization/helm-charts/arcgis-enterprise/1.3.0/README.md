...
| `install.ingress.customAnnotations` | Additional custom annotations added to the ingress service. | `[]`|
...
| `install.k8sAvailabilityTopologyKey` | If your Kubernetes cluster spans three or more availability zones change `kubernetes.io/hostname` to `topology.kubernetes.io/zone` | `kubernetes.io/hostname`|
| `install.clusterIngressControllerYamlFilename` | The filename in `user-inputs` containing the optional custom layer7 ingress definition. | |
...
- An [upgrade](https://enterprise-k8s.arcgis.com/en/latest/administer/upgrade-to-a-new-version.htm) is a new version of the software, with new features, improved functionality, and sometimes a different look and feel. For example, an upgrade moves the software from version 11.2 to version 11.3.
