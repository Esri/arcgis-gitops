# ArcGIS Server on Linux Deployment in AWS with Ansible

This experimental template provides GitHub Actions workflows for [ArcGIS Server deployment](https://enterprise.arcgis.com/en/server/latest/install/linux/welcome-to-the-arcgis-for-server-install-guide.htm) operations on Linux platforms. The workflows use [Ansible](https://www.ansible.com/) with [Ansible Collections for ArcGIS](../ansible_collections/README.md) to install and configure ArcGIS Enterprise applications.
...
GitHub Actions workflow **server-linux-ansible-aws-image** creates EC2 AMIs for ArcGIS Server deployment.

The workflow uses: [image](image/README.md) Packer template with [image.vars.json](../../config/aws/arcgis-server-linux-ansible/image.vars.json) config file.
...
4. Run server-linux-ansible-aws-image workflow using the branch.

### 3. Provision AWS Resources

GitHub Actions workflow **server-linux-ansible-aws-infrastructure** creates AWS resources for ArcGIS Server deployment.

The workflow uses [infrastructure](infrastructure/README.md) Terraform template with [infrastructure.tfvars.json](../../config/aws/arcgis-server-linux-ansible/infrastructure.tfvars.json) config file.
...
6. Run server-linux-ansible-aws-infrastructure workflow using the branch.
...
GitHub Actions workflow **server-linux-ansible-aws-application** configures or upgrades ArcGIS Server on EC2 instances.

The workflow uses [application](application/README.md) Terraform template with [application.tfvars.json](../../config/aws/arcgis-server-linux-ansible/application.tfvars.json) config file.
...
5. Commit the changes to the Git branch and push the branch to GitHub.
6. Run server-linux-ansible-aws-application workflow using the branch.
...
GitHub Actions workflow **server-linux-ansible-aws-test** tests ArcGIS Server deployment.
...
1. Run server-linux-ansible-aws-test workflow using the branch.
...
GitHub Actions workflow **server-linux-ansible-aws-backup** creates ArcGIS Server backups using [backup utility](https://enterprise.arcgis.com/en/server/latest/develop/linux/backup-utility.htm).

The workflow uses [backup](backup/README.md) Terraform template with [backup.tfvars.json](../../config/aws/arcgis-server-linux-ansible/backup.tfvars.json) config file.
...
1. Run server-linux-ansible-aws-backup workflow using the branch.

> To meet the required recovery point objective (RPO), schedule runs of server-linux-ansible-aws-backup workflow by configuring 'schedule' event in server-linux-ansible-aws-backup.yaml file.

### Restore from Backups

GitHub Actions workflow **server-linux-ansible-aws-restore** restores ArcGIS Server from backup using [restore utility](https://enterprise.arcgis.com/en/server/latest/develop/linux/restore-utility.htm).

The workflow uses [restore](restore/README.md) Terraform template with [restore.tfvars.json](../../config/aws/arcgis-server-linux-ansible/restore.tfvars.json) config file.
...
1. Run server-linux-ansible-aws-restore workflow using the branch.
...
GitHub Actions workflow **server-linux-ansible-aws-infrastructure** creates an AWS backup plan for the deployment that backs up the EC2 instances, S3 buckets, and the EFS file system created by the workflow in the enterprise's AWS Backup vault. The backup schedule is controlled by the CRON expression set in the "backup_schedule" property in the [infrastructure.tfvars.json](../../config/aws/arcgis-server-linux-ansible/infrastructure.tfvars.json) file.

If the deployment uses S3 and DynamoDB AWS storage services for ArcGIS Server configuration store, then the GitHub Actions workflow **server-linux-ansible-aws-application** enables versioning for the configuration store S3 bucket, tags the S3 bucket and DynamoDB table, and adds them to the deployment's backup plan.

The backup schedule is configured by a [CRON expression](https://docs.aws.amazon.com/scheduler/latest/UserGuide/schedule-types.html#cron-based) specified by the "backup_schedule" property and the number of days to retain backups is specified by the "backup_retention" property in [infrastructure.tfvars.json](../../config/aws/arcgis-server-linux-ansible/infrastructure.tfvars.json) config file.

#### Restoring from System-level Backups

GitHub Actions workflow **server-linux-ansible-aws-recover** restores the entire deployment from the system-level backup.

The workflow:

1. Runs [recover_deployment](../scripts/README.md#recover_deployment) python script with [recover.vars.json](../../config/aws/arcgis-server-linux-ansible/recover.vars.json) config file.
2. Runs [infrastructure](infrastructure/README.md) Terraform template with [infrastructure.tfvars.json](../../config/aws/arcgis-server-linux-ansible/infrastructure.tfvars.json) config file.
...
1. Run server-linux-ansible-aws-recover workflow using the main/default branch.
...
5. Run server-linux-ansible-aws-application workflow using the branch.

## Destroying Deployments

GitHub Actions workflow **server-linux-ansible-aws-destroy** destroys AWS resources created by server-linux-ansible-aws-image, server-linux-ansible-aws-infrastructure and server-linux-ansible-aws-application workflows.

The workflow uses [infrastructure](infrastructure/README.md) and [application](application/README.md) Terraform templates with [infrastructure.tfvars.json](../../config/aws/arcgis-server-linux-ansible/infrastructure.tfvars.json) and [application.tfvars.json](../../config/aws/arcgis-server-linux-ansible/application.tfvars.json) config files.
...
1. Run server-linux-ansible-aws-destroy workflow using the branch.

> server-linux-ansible-aws-destroy workflow does not delete the deployment's backups.
