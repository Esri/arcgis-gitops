...
GitHub Actions workflow **server-linux-aws-infrastructure** creates an AWS backup plan for the deployment that backs up the EC2 instances, S3 buckets, and the EFS file system created by the workflow in the enterprise's AWS Backup vault. The backup schedule is controlled by the CRON expression set in the "backup_schedule" property in the [infrastructure.tfvars.json](../../config/aws/arcgis-server-linux/infrastructure.tfvars.json) file.
