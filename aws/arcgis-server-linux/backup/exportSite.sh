...
# This script backs up the ArcGIS Server deployment's config store to the enterprise's backup S3 bucket.
#
...
ADMIN_URL="https://localhost:6443/arcgis/admin"
...
if [ -z "$TOKEN" ]; then
  TOKEN_ERROR=$(echo "$TOKEN_JSON" | jq -r '.messages[0] // .error.message // "Unknown token error"')
  echo "Error: Failed to generate a valid token. $TOKEN_ERROR"
  exit 1
fi
