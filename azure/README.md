...
Create an Azure storage account and a blob container in the account for the [Terraform backend](https://developer.hashicorp.com/terraform/language/settings/backends/azurerm). Assign the service principal the minimum data-plane access it requires for the backend, typically `Storage Blob Data Contributor` at the storage account or container scope.
