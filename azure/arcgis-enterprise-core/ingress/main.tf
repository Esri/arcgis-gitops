...
locals {
  app_gateway_subnet_id = var.subnet_id == null ? element(module.enterprise_core_info.app_gateway_subnets, 1) : var.subnet_id
...
resource "azurerm_resource_group" "deployment_rg" {
  name     = "${var.enterprise_id}-${var.ingress_id}-rg"
...
resource "tls_self_signed_cert" "ca_root_cert" {
  ...
  subject {
    ...
    organization = var.enterprise_id
  }
...
resource "azurerm_public_ip" "ingress" {
  name                = "${var.enterprise_id}-${var.ingress_id}-pip"
...
# Create the Application Gateway for the enterprise
resource "azurerm_application_gateway" "ingress" {
  ...
  dynamic "probe" {
    ...
    content {
      ...
      # If override_host is true, use the host name from the backend HTTP settings; otherwise, use the ingress FQDN.
      host                                      = probe.value.override_host ? null : var.ingress_fqdn
    ...
    content {
      ...
      rewrite_rule {
        ...
        request_header_configuration {
          ...
          header_value = "{http_req_host}" # var.ingress_fqdn
        }
      }
...
# Create a record in the private hosted zone that points the ingress FQDN 
# to the Application Gateway's frontend private IP.
resource "azurerm_private_dns_a_record" "ingress_fqdn" {
...
# Create a record in the public DNS zone that points the ingress FQDN
resource "azurerm_dns_a_record" "public_dns_entry" {
  ...
  name                = var.ingress_fqdn == var.dns_zone_name ? "@" : trimsuffix(var.ingress_fqdn, ".${var.dns_zone_name}")
...
# Store the ingress FQDN and web contexts in Key Vault
