...
locals {
  backend_address_pool_name      = "${var.enterprise_id}-beap"
  frontend_port_name             = "${var.enterprise_id}-feport"
  frontend_ip_configuration_name = "${var.enterprise_id}-feip"
  http_setting_name              = "${var.enterprise_id}-be-htst"
  listener_name                  = "${var.enterprise_id}-httplstn"
  request_routing_rule_name      = "${var.enterprise_id}-rqrt"
  redirect_configuration_name    = "${var.enterprise_id}-rdrcfg"
}
...
resource "azurerm_resource_group" "cluster_rg" {
  name     = "${var.enterprise_id}-k8s-cluster"
  location = var.azure_region
}

# Create an AKS cluster
resource "azurerm_kubernetes_cluster" "enterprise_cluster" {
  name                      = var.enterprise_id
  location                  = azurerm_resource_group.cluster_rg.location
  resource_group_name       = azurerm_resource_group.cluster_rg.name
  kubernetes_version        = var.kubernetes_version
  dns_prefix                = var.enterprise_id
  sku_tier                  = "Standard"
  ...
  default_node_pool {
    ...
    vnet_subnet_id              = var.subnet_id != null ? var.subnet_id : module.enterprise_core_info.private_subnets[0]
...
resource "azurerm_key_vault_secret" "aks_identity_client_id" {
  ...
  value        = azurerm_kubernetes_cluster.enterprise_cluster.kubelet_identity[0].client_id
...
resource "azurerm_key_vault_secret" "aks_identity_principal_id" {
  ...
  value        = azurerm_kubernetes_cluster.enterprise_cluster.kubelet_identity[0].object_id
  key_vault_id = module.enterprise_core_info.vault_id
}

# Assign Storage Blob Data Contributor role to the AKS cluster identity
resource "azurerm_role_assignment" "storage_blob_data_contributor" {
  principal_id                     = azurerm_kubernetes_cluster.enterprise_cluster.kubelet_identity[0].object_id
  role_definition_name             = "Storage Blob Data Contributor"
  scope                            = module.enterprise_core_info.storage_account_id
...
resource "null_resource" "update_kubeconfig" {
  ...
  provisioner "local-exec" {
    command = "az aks get-credentials --resource-group ${azurerm_resource_group.cluster_rg.name} --name ${azurerm_kubernetes_cluster.enterprise_cluster.name} --overwrite-existing"
  }

  depends_on = [
    azurerm_kubernetes_cluster.enterprise_cluster
  ]
}
...
module "container_registry" {
  ...
  vnet_id                     = module.enterprise_core_info.vnet_id
...
module "alb_controller" {
  ...
  azure_region        = var.azure_region
  resource_group_name = azurerm_resource_group.cluster_rg.name
  cluster_name        = azurerm_kubernetes_cluster.enterprise_cluster.name
  alb_subnet_id       = module.enterprise_core_info.app_gateway_subnets[0]
