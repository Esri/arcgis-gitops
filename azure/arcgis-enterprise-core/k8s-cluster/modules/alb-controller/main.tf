...
resource "azurerm_role_assignment" "aks_cluster_reader" {
  ...
  # scope                            = data.azurerm_kubernetes_cluster.enterprise_cluster.node_resource_group_id
...
resource "azurerm_federated_identity_credential" "azure_alb_identity" {
  ...
  resource_group_name = var.resource_group_name
  issuer              = data.azurerm_kubernetes_cluster.enterprise_cluster.oidc_issuer_url
