...
resource "azurerm_container_registry" "cluster_acr" {
  ...
  name                          = "${replace(var.enterprise_id, "-", "")}${random_id.container_registry_suffix.hex}"
...
resource "azurerm_container_registry_credential_set" "credential_set" {
  ...
  authentication_credentials {
    username_secret_id = "${module.enterprise_core_info.vault_uri}secrets/cr-user"
    password_secret_id = "${module.enterprise_core_info.vault_uri}secrets/cr-password"
  }
}
...
resource "azurerm_role_assignment" "pull_through_cache" {
  ...
  scope                            = module.enterprise_core_info.vault_id
