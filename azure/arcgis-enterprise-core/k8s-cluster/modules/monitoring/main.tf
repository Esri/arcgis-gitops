...
data "azurerm_kubernetes_cluster" "enterprise_cluster" {
...
resource "azurerm_monitor_data_collection_rule_association" "dcra" {
  ...
  target_resource_id      = data.azurerm_kubernetes_cluster.enterprise_cluster.id
...
#   name                = "${var.enterprise_id}-default-dce"
#   resource_group_name = azurerm_resource_group.cluster_rg.name
...
resource "azurerm_monitor_alert_prometheus_rule_group" "node_recording_rules_rule_group" {
  ...
  interval            = "PT1M"
  scopes              = [azurerm_monitor_workspace.prometheus.id, data.azurerm_kubernetes_cluster.enterprise_cluster.id]
