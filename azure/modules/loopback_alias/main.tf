...
resource "null_resource" "mount" {
  ...
  provisioner "local-exec" {
    ...
    command = "python -m az_run_shell_script -s ${var.site_id} -d ${var.deployment_id} -m ${join(",", var.machine_roles)} -f ${path.module}/scripts/SetLoopbackAlias.ps1 -v ${data.azurerm_key_vault.site_vault.name}" 
  }
}
