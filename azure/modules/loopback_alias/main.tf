  ...
  provisioner "local-exec" {
    environment = {
      JSON_PARAMETERS = base64encode(jsonencode({
        AliasFQDN = var.alias_fqdn
      }))
    }

    command = "python -m az_run_shell_script -s ${var.enterprise_id} -d ${var.deployment_id} -m ${join(",", var.machine_roles)} -f ${path.module}/scripts/SetLoopbackAlias.ps1 -v ${data.azurerm_key_vault.enterprise_vault.name}"
  }
}
